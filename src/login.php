...
function IFPOST($request_data){
	if(empty($request_data['User_ID'])){
		echo json_encode(array('Type'=>1,'Result'=>array('Errmsg'=>"1.未输入账号！")));
	}else if(empty($request_data['User_Password'])){
		echo json_encode(array('Type'=>1,'Result'=>array('Errmsg'=>"2.未输入密码！")));
	}else{
		...
		$count=count($rs);
		if($count==0){
			echo json_encode(array('Type'=>1,'Result'=>array('Errmsg'=>"3.ID不存在！")));
		}else{
			if($rs['User_Password']==$PW){
				...
				echo json_encode(array('Type'=>1,'Result'=>array('Errmsg'=>"4.密码错误！")));
			}
		}
	}
}
...
function IFDELETE($request_data){
	...
	if($type==0){
		...
		echo json_encode(array('Type'=>1,'Result'=>array('Errmsg'=>"1.退出类型错误！")));
	}
}
?>